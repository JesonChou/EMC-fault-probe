...
>
> **Current status**: the foundation is complete — data collected from three sources (papers, web, online LLMs), cleaned, LLM-extracted, and deduplicated into 151 published entries; the PyQt6 query application and ChromaDB RAG implementation are preserved as experiments. The vector database is generated by `experiments/rag/embedding_test.py` into `experiments/rag/emc_vector_db/`; the Web frontend, Python backend, and Agent runtimes are being rebuilt under the new architecture.
